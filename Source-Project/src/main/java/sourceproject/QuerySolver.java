...
//    private static String DECRYPTED_KEY = decryptAPIKey();
//
...
//
//        ByteBuffer plainTextKey = AWSKMSClientBuilder.defaultClient().decrypt(request).getPlaintext();
...
//    }
    ...
    public QuerySolver() {
        ...
        try {
            InputStream modelIn = QuerySolver.class.getResourceAsStream("/en-token.bin");
            tokenmodel = new TokenizerModel(modelIn);

            InputStream modelIn2 = QuerySolver.class.getResourceAsStream("/en-pos-maxent.bin");
            tagmodel = new POSModel(modelIn2);

            InputStream modelIn3 = QuerySolver.class.getResourceAsStream("/en-chunker.bin");
            chunkmodel = new ChunkerModel(modelIn3);

            InputStream modelIn4 = QuerySolver.class.getResourceAsStream("/en-sent.bin");
...
//PART 1
    ...
    */
    ...
    private String buildSearchString(String searchString, int start) {
        String toSearch = searchURL + "key=" + API_KEY + "&cx=" + SEARCH_ENGINE_ID + "&q=";
...
//        System.out.println();
    ...
    public TreeMap<Integer, Double> scrapePages(String query, ArrayList<String> listOfURLs) {
        ...
        System.out.println("Number of URLS: " + listOfURLs.size());
...
//PART 3
    ...
    private HashMap<Integer, AbstractMap.SimpleImmutableEntry<ArrayList<Double>, Integer>> makeHistogram(TreeMap<Integer, Double> inputMap,
                                                                                                         double error) {
        assert inputMap != null && !inputMap.isEmpty() && error > 0 && error < 1 : "Invalid inputs";

        System.out.println(inputMap.size());
        HashMap<Integer, AbstractMap.SimpleImmutableEntry<ArrayList<Double>, Integer>> histogram = new HashMap<Integer, AbstractMap.SimpleImmutableEntry<ArrayList<Double>, Integer>>();
        ...
        for (int valueIndex = 0; valueIndex < origValues.size(); valueIndex++) {
            ...
            if (histogram.containsKey(bucketNum)) {
                ...
                lst.add(value);
                histogram.put(bucketNum, new AbstractMap.SimpleImmutableEntry<ArrayList<Double>, Integer>(lst, histogram.get(bucketNum).getValue() + entryList.get(valueIndex).getKey()));
            }
            ...
            //mapping a list (which just contains the value at valueIndex) to the score of the source
                ...
                AbstractMap.SimpleImmutableEntry<ArrayList<Double>, Integer> p = new AbstractMap.SimpleImmutableEntry<ArrayList<Double>, Integer>(newLst, entryList.get(valueIndex).getKey());
                histogram.put(bucketNum, p);
            }
        }
        return histogram;
    }
...
//PART 4
    ...
    private double getValue(HashMap<Integer, AbstractMap.SimpleImmutableEntry<ArrayList<Double>, Integer>> inputMap) {
        if (inputMap.isEmpty())
            throw new IllegalArgumentException("Map is empty");

        //inputMap.values() gets a Collection of all the values that are mapped to in the map
        ArrayList<AbstractMap.SimpleImmutableEntry<ArrayList<Double>, Integer>> mapValues = new ArrayList<AbstractMap.SimpleImmutableEntry<ArrayList<Double>, Integer>>(inputMap.values());
        int maxIndex = -1;
        double maxScore = Integer.MIN_VALUE;

        //Finds the lowest total score and the index at which it was found
        for (int i = 0; i < mapValues.size(); i++) {
            AbstractMap.SimpleImmutableEntry<ArrayList<Double>, Integer> pair = mapValues.get(i);
    ...
    //-----------------------------------------------------------------------------------------------------------------------------
    //PART 5
    public double solution(String query, double error, String API_KEY, String SEARCH_ENGINE_ID) {
        this.API_KEY = API_KEY;
        this.SEARCH_ENGINE_ID = SEARCH_ENGINE_ID;
        HashMap<Integer, AbstractMap.SimpleImmutableEntry<ArrayList<Double>, Integer>> indexToPairMap = null;
...
//        QuerySolver main = new QuerySolver();
//        for (int i = 0; i < 20; i++) {
